#!/usr/bin/env bash
# The app itself is stateless (Supabase holds all real data) — "backup" here
# means the running config (.env) and a portable copy of the exact image
# currently in production, so a fresh VPS can be brought back up without
