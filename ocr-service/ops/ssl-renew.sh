...
#
# No-op if DOMAIN isn't set yet — safe to add to a cron job before you have
# a domain, and to leave running once you do (certbot only renews when the
# cert is actually close to expiry).
...
if compose --profile proxy run --rm certbot test -f "$cert_path" >/dev/null 2>&1; then
  ...
  log_warn "(template included at the bottom of that file) and redeploy with --with-proxy."
fi
