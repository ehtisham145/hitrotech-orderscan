#!/usr/bin/env bash
# Watchdog: polls /health on an interval; after N consecutive failures it
...
check_once() {
  ...
  if "$OPS_DIR/health.sh" --retries 1 --quiet; then
    fail_count=0
    stats="$(docker stats --no-stream --format '{{.CPUPerc}} {{.MemUsage}}' orderscan-ocr 2>/dev/null || echo 'n/a')"
    ...
    if [ "$fail_count" -ge "$threshold" ]; then
      log_err "Threshold reached — restarting orderscan-ocr"
      echo "$ts RESTART" >>"$log_file"
      compose restart ocr || true
      fail_count=0
    fi
  fi
}
