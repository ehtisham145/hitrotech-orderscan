...
# Resolve ocr-service/ regardless of where the script was invoked from.
OPS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$OPS_DIR/.." && pwd)"
STATE_DIR="$OPS_DIR/.state"
mkdir -p "$STATE_DIR"

IMAGE_NAME="orderscan-ocr"
...
# Load .env into the current shell (for scripts that need OCR_PORT etc. directly).
load_env() {
  require_env_file
  set -a
