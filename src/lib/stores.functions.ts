...
export async function listStoresCore(context: ServerContext) {
...
export const listStores = createServerFn({ method: "GET" })
  ...
  .handler(({ context }) => listStoresCore(context));

export async function createStoreCore(
  data: { code: string; label?: string | null },
...
export const createStore = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) => createStoreCore(data, context));

export async function updateStoreCore(
  data: { id: string; code: string; label?: string | null },
  context: ServerContext,
) {
  const wsId = await assertActiveWorkspaceRole(context.supabase, context.userId, ["owner", "admin"]);
  // Same normalisation and limits as createStoreCore — a code edited here has
  // to end up in exactly the shape a created one would, or the two paths drift.
  const code = data.code.trim().toUpperCase();
  if (!code) throw new Error("Store code is required");
  if (code.length > 32) throw new Error("Store code is too long");
  const label = data.label?.trim() || null;
...
export const updateStore = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) => updateStoreCore(data, context));

export async function deleteStoreCore(data: { id: string }, context: ServerContext) {
  const wsId = await assertActiveWorkspaceRole(context.supabase, context.userId, ["owner", "admin"]);
...
export const deleteStore = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) => deleteStoreCore(data, context));
