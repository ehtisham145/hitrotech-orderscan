...
// BUG FIX: the old formula was `input.slice(0, 8) + "01"`, which only
// produces a valid date when `input` is already a full "YYYY-MM-DD" (10
// chars) — slicing 8 characters off a 7-char "YYYY-MM" input (exactly what
// an <input type="month"> gives you) returns the whole "YYYY-MM" unchanged,
// so the "01" gets appended with no separating dash ("2026-0901"), which
// isn't a parseable date at all. Reproduced directly:
// "2026-09".slice(0, 8) + "01" -> "2026-0901" (broken)
// "2026-09-15".slice(0, 8) + "01" -> "2026-09-01" (looks fine, by luck of
// matching lengths — which is exactly why this went unnoticed).
// Fixed by always taking just the "YYYY-MM" prefix and appending "-01"
// explicitly, which is correct regardless of how long the input string is.
...
// BUG FIX: this query had no workspace filter at all — any authenticated
// user could read every workspace's audit trail (who did what, when,
// including edited/deleted-record details).
export async function listAuditLogsCore(
  ...
  if (userIds.length > 0) {
    const { data: profiles, error: profErr } = await context.supabase.from("profiles").select("id, email").in("id", userIds);
...
export const listAuditLogs = createServerFn({ method: "GET" })
  ...
  .handler(({ data, context }) => listAuditLogsCore(data, context));

// ---------------------------------------------------------------
// ANOMALIES
// ---------------------------------------------------------------

export async function listAnomaliesCore(data: { month?: string }, context: ServerContext) {
  ...
  // BUG FIX: this query's error was discarded — a failed scan silently
  // reported "no repeated-phone anomalies" instead of failing.
...
export const listAnomalies = createServerFn({ method: "GET" })
  ...
  .handler(({ data, context }) => listAnomaliesCore(data, context));

// ---------------------------------------------------------------
// MONTH LOCKS
// ---------------------------------------------------------------

// BUG FIX: no workspace filter — every workspace's lock history leaked to
// any authenticated user.
export async function listMonthLocksCore(context: ServerContext) {
  const wsId = await requireActiveWorkspaceId(context.supabase, context.userId);
...
export const listMonthLocks = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(({ context }) => listMonthLocksCore(context));

// BUG FIX: `.eq("month", m)` alone with no workspace filter — two workspaces
// locking the same calendar month (an ordinary thing to happen) would make
// this query match more than one row, and .maybeSingle() throws on that
// rather than silently returning the wrong workspace's lock. Scoped now.
export async function isMonthLockedCore(data: { month: string }, context: ServerContext) {
  const wsId = await requireActiveWorkspaceId(context.supabase, context.userId);
  const m = monthStart(data.month);
...
export const isMonthLocked = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { month: string }) => input)
  .handler(({ data, context }) => isMonthLockedCore(data, context));

export async function lockMonthCore(data: { month: string; notes?: string | null }, context: ServerContext) {
  ...
  if (auditErr) console.error("[reliability] Could not write audit log for month.locked:", auditErr.message);
  return { ok: true };
}
...
export const lockMonth = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) => lockMonthCore(data, context));

export async function unlockMonthCore(data: { month: string }, context: ServerContext) {
  const wsId = await assertActiveWorkspaceRole(context.supabase, context.userId, [...LOCK_ROLES]);
  const m = monthStart(data.month);
  ...
  if (auditErr) console.error("[reliability] Could not write audit log for month.unlocked:", auditErr.message);
  return { ok: true };
}
...
export const unlockMonth = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { month: string }) => input)
  .handler(({ data, context }) => unlockMonthCore(data, context));

/** Recent lock/unlock events for the active workspace, most recent first. */
export async function listMonthLockHistoryCore(data: { month?: string; limit?: number }, context: ServerContext) {
  const wsId = await requireActiveWorkspaceId(context.supabase, context.userId);
...
export const listMonthLockHistory = createServerFn({ method: "GET" })
  ...
  .handler(({ data, context }) => listMonthLockHistoryCore(data, context));

// ---------------------------------------------------------------
// RECONCILIATION — compare payout snapshot vs live activation count
// ---------------------------------------------------------------

// BUG FIX: none of the three queries' errors were checked. A failed query
// here silently produced wrong or incomplete reconciliation numbers — on a
// money report, a failure and a genuinely-reconciled month looked the same.
export async function getReconciliationCore(data: { month: string }, context: ServerContext) {
  const m = monthStart(data.month);
  const wsId = await requireActiveWorkspaceId(context.supabase, context.userId);
...
export const getReconciliation = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { month: string }) => input)
  .handler(({ data, context }) => getReconciliationCore(data, context));

// ---------------------------------------------------------------
// MONTHLY BACKUP EXPORT
// ---------------------------------------------------------------

export async function exportMonthlyBackupCore(data: { month: string }, context: ServerContext) {
  const m = monthStart(data.month);
  ...
  const { data: allowed, error: allowedErr } = await context.supabase.rpc("is_manager_or_admin", { _user_id: context.userId });
  ...
  // BUG FIX: none of these five errors were checked — a backup is exactly
  // the feature that must not silently ship incomplete/wrong data as if it
  // were a genuinely empty month.
  if (extRes.error) throw new Error(extRes.error.message);
  if (payoutsRes.error) throw new Error(payoutsRes.error.message);
  if (partnersRes.error) throw new Error(partnersRes.error.message);
  ...
  // BUG FIX: this insert's error was fully discarded. The file itself is
  // already uploaded and about to be signed/returned by this point, so a
  // failure here is logged (the backup still works) rather than thrown —
  // it just means this export won't show up in a "past backups" list.
  const { error: reportErr } = await supabaseAdmin.from("generated_reports").insert({
    user_id: context.userId,
  ...
  if (reportErr) console.error("[reliability] Could not record generated_reports entry for backup:", reportErr.message);
...
export const exportMonthlyBackup = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { month: string }) => input)
  .handler(({ data, context }) => exportMonthlyBackupCore(data, context));
