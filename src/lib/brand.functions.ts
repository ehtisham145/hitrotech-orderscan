...
export async function listBrandsCore(context: ServerContext) {
...
export const listBrands = createServerFn({ method: "GET" })
  ...
  .handler(({ context }) => listBrandsCore(context));

export async function upsertBrandCore(data: { id?: string; name: string; operator?: string | null; active?: boolean; notes?: string | null }, context: ServerContext) {
  const wsId = await assertActiveWorkspaceRole(context.supabase, context.userId, [...WRITE_ROLES]);
  if (data.id) {
    const { id, ...rest } = data;
    // .select("id") so an edit that matches nothing — wrong workspace, or RLS
    // refusing — is reported instead of returning ok for a write that never
    // happened.
    const { data: updated, error } = await context.supabase
      .from("brands").update(rest).eq("workspace_id", wsId).eq("id", id).select("id");
    if (error) return { ok: false as const, error: error.message };
    if (!updated?.length) return { ok: false as const, error: "Brand not found in this workspace" };
...
export const upsertBrand = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) => upsertBrandCore(data, context));

export async function deleteBrandCore(data: { id: string }, context: ServerContext) {
  const wsId = await assertActiveWorkspaceRole(context.supabase, context.userId, [...WRITE_ROLES]);
...
export const deleteBrand = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) => deleteBrandCore(data, context));

export async function listBrandSlabsCore(data: { brand_id: string }, context: ServerContext) {
...
export const listBrandSlabs = createServerFn({ method: "GET" })
  ...
  .handler(({ data, context }) => listBrandSlabsCore(data, context));

export async function upsertBrandSlabCore(data: { id?: string; brand_id: string; min_count: number; max_count: number | null; rate_pkr: number; active: boolean }, context: ServerContext) {
  const wsId = await assertActiveWorkspaceRole(context.supabase, context.userId, [...WRITE_ROLES]);
  if (data.id) {
    const { id, ...rest } = data;
    const { data: updated, error } = await context.supabase
      .from("brand_slabs").update(rest).eq("workspace_id", wsId).eq("id", id).select("id");
    if (error) return { ok: false as const, error: error.message };
    if (!updated?.length) return { ok: false as const, error: "Slab not found in this workspace" };
...
export const upsertBrandSlab = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) => upsertBrandSlabCore(data, context));

export async function deleteBrandSlabCore(data: { id: string }, context: ServerContext) {
  const wsId = await assertActiveWorkspaceRole(context.supabase, context.userId, [...WRITE_ROLES]);
...
export const deleteBrandSlab = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { id: string }) => input)
  .handler(({ data, context }) => deleteBrandSlabCore(data, context));
 ...
 */
export async function getAgencyEarningsCore(data: { month?: string; brand_id?: string }, context: ServerContext) {
  ...
  // Every one of these was previously read as `.data ?? []` with the error
  // discarded. A failed query then looked exactly like a month with no
  // activity, so the earnings report showed zero revenue and zero cost instead
  // of saying anything had gone wrong — the worst possible failure mode for a
  // number someone bills against.
  for (const [what, res] of [
    ["brands", brandsRes], ["activations", extRes],
    ["partners", partnersRes], ["employees", employeesRes],
  ] as const) {
    if (res.error) throw new Error(`Could not load ${what} for the agency report: ${res.error.message}`);
  }
...
export const getAgencyEarnings = createServerFn({ method: "GET" })
  ...
  .handler(({ data, context }) => getAgencyEarningsCore(data, context));

/** Freeze the month's numbers into a receivable the agency can chase. */
export async function saveBrandInvoiceCore(data: { brand_id: string; month: string }, context: ServerContext) {
  const wsId = await assertActiveWorkspaceRole(context.supabase, context.userId, [...WRITE_ROLES]);
  const month = monthStart(data.month);
...
export const saveBrandInvoice = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) => saveBrandInvoiceCore(data, context));

export async function markBrandInvoiceReceivedCore(data: { id: string; received: boolean; payment_reference?: string | null; notes?: string | null }, context: ServerContext) {
...
export const markBrandInvoiceReceived = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) => markBrandInvoiceReceivedCore(data, context));
