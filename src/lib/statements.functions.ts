...
// BUG FIX (third copy of this bug found in the codebase — see reliability.
// functions.ts and month-close.server.ts for the same fix): the old formula
// `input.slice(0, 8) + "01"` only produces a valid date for a 10-char
// "YYYY-MM-DD" input. A 7-char "YYYY-MM" input (what <input type="month">
// sends) comes out as "2026-0901" — no separating dash, not parseable.
...
export async function getPartnerStatementCore(data: { partner_id: string; month?: string }, context: ServerContext) {
  ...
  // BUG FIX: none of these four errors was checked — on a money report,
  // any one of them failing silently produced wrong or empty numbers
  // instead of failing. partnerRes.error in particular used to be
  // indistinguishable from "no such partner".
...
export const getPartnerStatement = createServerFn({ method: "GET" })
  ...
  .handler(({ data, context }) => getPartnerStatementCore(data, context));

export async function getPartnerHistoryCore(data: { partner_id: string; months?: number }, context: ServerContext) {
  ...
  // BUG FIX: same as getPartnerStatementCore — none of these three were checked.
  if (partnerRes.error) throw new Error(partnerRes.error.message);
  if (actRes.error) throw new Error(actRes.error.message);
  if (payoutRes.error) throw new Error(payoutRes.error.message);
...
export const getPartnerHistory = createServerFn({ method: "GET" })
  ...
  .handler(({ data, context }) => getPartnerHistoryCore(data, context));
