...
const inputSchema = z.object({ workspaceId: z.string().uuid().optional().nullable() });

export async function getBusinessTrendsCore(data: z.infer<typeof inputSchema>, context: ServerContext) {
  const { supabase, userId } = context;

  let wsId = data.workspaceId || null;
  if (wsId) {
    // BUG FIX: a caller-supplied workspaceId was used with *no* authorization
    // check at all — any authenticated user could pass any workspace's uuid
    // and get back that workspace's full commission/partner/revenue trends.
    // Same membership-or-super-admin check setActiveWorkspaceCore uses.
    const [membershipRes, superRes] = await Promise.all([
      supabase.from("workspace_members").select("workspace_id").eq("user_id", userId).eq("workspace_id", wsId).maybeSingle(),
    ...
    const { data: p, error: profErr } = await supabase.from("profiles").select("active_workspace_id").eq("id", userId).maybeSingle();
  ...
  // BUG FIX: none of these three errors was checked — a failed query
  // silently produced empty/wrong charts instead of failing.
...
export const getBusinessTrends = createServerFn({ method: "POST" })
  ...
  .handler(({ context, data }) => getBusinessTrendsCore(data, context));
